...
export default function Home() {
  ...
  async function translateAll() {
    ...
    for (let i = 0; i < ALL_LANGUAGES.length; i += BATCH_SIZE) {
      batches.push(ALL_LANGUAGES.slice(i, i + BATCH_SIZE));
    }
    setBatchTotal(batches.length);
    setBatchProgress(0);

    const allResults = {};
    const GROUP = 3; // 3 parallel calls at a time
    for (let g = 0; g < batches.length; g += GROUP) {
      ...
      const promises = group.map(batch =>
        ...
        }).then(r => r.json()).then(d => d.translations || {})
      );
  ...
  async function applyAll() {
    ...
    for (const [, trans] of Object.entries(allTranslations)) {
      if (trans.langCode && trans.title) {
        localizations[trans.langCode] = { title: trans.title, description: trans.description || '' };
      }
    }
    try {
      ...
      const data = await res.json();
      setAllStatus(data.success ? `ok:${data.count}` : 'error');
  ...
  const filteredVideos = channelFilter === 'Todos' ? videos : videos.filter(v => v.snippet.channelTitle === channelFilter);
  ...
  const s = {
    ...
    primaryBtn: { background: '#8b5cf6', color: 'white', border: 'none', borderRadius: '10px', padding: '12px 22px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', textDecoration: 'none', display: 'inline-block' },
    applyBtn: { background: '#059669', color: 'white', border: 'none', borderRadius: '10px', padding: '12px 22px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' },
    allBtn: { background: 'linear-gradient(135deg,#f59e0b,#f43f5e)', color: 'white', border: 'none', borderRadius: '10px', padding: '12px 22px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' },
    ...
    chipRow: { display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '20px' },
    chip: { padding: '6px 14px', borderRadius: '999px', fontSize: '13px', fontWeight: '500', cursor: 'pointer', border: '1px solid #34303e', background: '#1c1a22', color: '#a29db3' },
    ...
    badge: { display: 'inline-block', fontSize: '11px', fontWeight: '600', padding: '2px 8px', borderRadius: '999px', marginBottom: '4px' },
    ...
    progress: { background: '#26232e', borderRadius: '999px', height: '8px', overflow: 'hidden', marginBottom: '12px' },
  ...
  if (!authenticated) return (
    <div style={{ ...s.page, ...s.center }}>
      ...
      <div style={{ textAlign: 'center', maxWidth: '480px' }}>
        ...
        <p style={{ color: '#a29db3', fontSize: '14px', margin: '0 0 24px' }}>Liga a tua conta do YouTube para traduzires automaticamente os teus vídeos para qualquer língua.</p>
      ...
      <div style={s.wrap}>
        ...
        {!selectedVideo ? (
          <div>
            <div style={s.chipRow}>
              {channels.map(ch => (
                <button key={ch} onClick={() => setChannelFilter(ch)} style={{ ...s.chip, background: channelFilter === ch ? (ch === 'Todos' ? '#8b5cf6' : colorMap[ch] || '#8b5cf6') : '#1c1a22', color: channelFilter === ch ? 'white' : '#a29db3', border: `1px solid ${channelFilter === ch ? 'transparent' : '#34303e'}` }}>{ch}</button>
              ))}
            </div>
          ...
          <div>
            <div style={s.selBox}>
              ...
              <div>
                <div style={{ ...s.badge, background: (colorMap[selectedVideo.snippet.channelTitle] || '#8b5cf6') + '22', color: colorMap[selectedVideo.snippet.channelTitle] || '#8b5cf6', marginBottom: '6px' }}>{selectedVideo.snippet.channelTitle}</div>
              ...
              {batchRunning && (
                <div>
                  <div style={s.progress}>
                    <div style={{ ...s.progressBar, width: `${(batchProgress / batchTotal) * 100}%` }} />
                  </div>
                  <div style={{ fontSize: '13px', color: '#a29db3', marginBottom: '12px' }}>
                    A traduzir lote {batchProgress}/{batchTotal}... ({Math.round((batchProgress / batchTotal) * ALL_LANGUAGES.length)} línguas prontas)
                  </div>
                </div>
              )}
              <div style={s.actions}>
                {!allTranslations && !batchRunning && (
                  <button onClick={translateAll} style={s.allBtn}>⚡ Traduzir para TODAS ({ALL_LANGUAGES.length} línguas)</button>
                )}
                {allTranslations && !allStatus && (
                  <>
                    <div style={{ fontSize: '13px', color: '#22d3ee', marginRight: '8px' }}>✓ {Object.keys(allTranslations).length} línguas traduzidas!</div>
                    <button onClick={applyAll} disabled={applyingAll} style={{ ...s.applyBtn, opacity: applyingAll ? 0.6 : 1 }}>
                      {applyingAll ? 'A aplicar no YouTube...' : '✓ Aplicar TODAS no YouTube'}
                    </button>
                    <button onClick={translateAll} style={{ ...s.backBtn }}>Repetir</button>
                  </>
                )}
                {allStatus.startsWith('ok:') && <span style={s.statusOk}>✓ {allStatus.split(':')[1]} traduções aplicadas no YouTube!</span>}
                {allStatus === 'error' && <span style={s.statusErr}>Erro ao aplicar. Tenta novamente.</span>}
              </div>
            </div>
            ...
            <div style={s.actions}>
              ...
              {status === 'applied' && <span style={s.statusOk}>✓ Aplicado com sucesso!</span>}
